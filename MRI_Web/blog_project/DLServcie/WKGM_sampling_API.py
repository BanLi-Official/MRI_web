...
def write_Data(model_num,psnr,ssim,mse,name,times,path):
    ...
    with open(osp.join(path,filedir),"w+") as f:#a+
        f.writelines(str(model_num)+' '+'['+str(round(psnr, 2))+' '+str(round(ssim, 4))+' '+str(round(mse, 4))+']'+ str(times))
        f.write('\n')
        
def write_Data2(psnr,ssim,mse,name,path):
    filedir=name+"PC_SAKE.txt"
    with open(osp.join(path,filedir),"a+") as f:#a+
        f.writelines('['+str(round(psnr, 2))+' '+str(round(ssim, 4))+' '+str(round(mse, 4))+']')
        f.write('\n')
...
def get_pc_sampler(sde, shape, predictor, corrector, inverse_scaler, snr,
  ...
  def pc_sampler(model,test_data,img_name,save_path,my_ori,my_mask):
    ...
    with torch.no_grad():
      ...
      coil = 8
      ori_data1 = np.zeros([256,256,coil],dtype=np.complex64)
      print("ori_data1.shape=",ori_data1.shape)
      
      print("my_ori.shape=",my_ori.shape)
      #exit()
      ori_data1=my_ori
      #ori_data1 = test_data['T1_img'].squeeze().cpu().numpy()
      ori_data = np.zeros((coil, 256, 256), dtype=np.complex64)
      for i in range(coil):
        #ori_data[i,:,:] = np.fft.fftshift(np.fft.ifft2(ori_data1[:,:,i]))# max: 3820.8044
        #ori_data[i,:,:] = np.fft.fftshift((ori_data1[:,:,i]))# max: 3820.8044
        ori_data[i,:,:]= (ori_data1[:,:,i])
      ...
      ori_data_sos1 = np.sqrt(np.sum(np.square(np.abs(ori_data1)),axis=2)) 
      write_images(abs(ori_data_sos),osp.join(save_path,img_name+'ori'+'.png'))
      write_images(abs(ori_data_sos1),osp.join(save_path,img_name+'ori1'+'.png'))
      
      #======== mask
      mask = np.zeros((coil,256,256))
      mask_item = my_mask
      #mask_item = 1 - mask_item
      # mask_item = io.loadmat('./datasets_lw/实验室祖传mask和weight/mask/mask_gen/possion/possion_256_acs24_准/possion_256_acs24_r12.mat')['mask']
      #mask_item = io.loadmat('/media/mri/data/xx/XX-T2-WKGM/parallel_inputdata/mask/poisson/6.mat')['mask'] 
      #mask_item = io.loadmat('/home/lqg/桌面/ncsn++/parallel_inputdata/contract_mask/random2D/4.mat')['mask']   
      #mask_item = io.loadmat('/home/lqg/桌面/KI-EBM并行成像/input_data/mask/mask_cart_030.mat')['mask_cart_030']         
      for i in range(coil):
        mask[i,:,:] = mask_item
      ...
      #ww = io.loadmat('./input_data/weight1_PeiBrain.mat')['weight']
      #ww = io.loadmat('./input_data/weight1_GEBrain.mat')['weight']
      ww = io.loadmat('./input_data/weight10.mat')['weight']
      # ww = io.loadmat('./input_data/weight_fastmri_0.5_0.04.mat')['weight']
      # ww = np.array(ww)
      # ww = [32:288, 32:288]
      ...
      write_images(abs(zeorfilled_data_sos),osp.join(save_path, img_name+'Zeorfilled.png'))
      ...
      for indx in range(300):
        ...
        for i in range(coil):       
          ...
          #k_complex2[i, :, :] = np.fft.fftshift(k_complex2[i, :, :])    #zhe
        write_images(abs(k_complex2[1,:,:]), osp.join(save_path, 'k_p' + '.png'))
        ...
        write_images(abs(k_complex2[1, :, :]), osp.join(save_path, 'k_c' + '.png'))

        NCSNpp_Image_sos = np.sqrt(np.sum(np.square(np.abs(NCSNpp_Image)), axis=0))
        write_images(abs(NCSNpp_Image_sos), osp.join(save_path, img_name + 'wkgm' + '.png'))
        io.savemat(osp.join(save_path, img_name + 'wkgm.mat'), {'wkgm': NCSNpp_Image})

        NCSNpp_Image_sos = NCSNpp_Image_sos / np.max(np.abs(NCSNpp_Image_sos))
        psnr_w = compare_psnr(255 * abs(NCSNpp_Image_sos), 255 * abs(ori_data_sos), data_range=255)
        ssim_w = compare_ssim(abs(NCSNpp_Image_sos), abs(ori_data_sos), data_range=1)
        mse_w = compare_mse(abs(NCSNpp_Image_sos), abs(ori_data_sos)) * 10000
        print('wkgm_psnr: ', psnr_w, 'wkgm_ssim: ', ssim_w, 'wkgm_mse: ', mse_w)
        ...
        rec_Image=Input_Imag
        Krec =  k_complex3

        write_images(abs(Krec[:, :, 1]), osp.join(save_path, 'Krec' + '.png'))
        #=======================================================  
        rec_Image_sos = np.sqrt(np.sum(np.square(np.abs(rec_Image)),axis=2))
        write_images(abs(rec_Image_sos), osp.join(save_path, img_name + 'sake' + '.png'))
        io.savemat(osp.join(save_path, img_name + 'sake.mat'), {'sake': rec_Image})
        Rec_Image_sos = np.copy(rec_Image_sos)
        ...
        print('sake_psnr: ', psnr, 'sake_ssim: ', ssim, 'sake_mse: ', mse)
        ...
        for i in range(size[2]):
          #K_end[:,:,i] = np.sqrt(fctr)*np.fft.fftshift(Krec[:, :, i])   #zhe
